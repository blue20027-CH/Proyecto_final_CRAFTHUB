...
def obtener_productos(categoria: Optional[str] = None, busqueda: Optional[str] = None):
    """
    Obtiene la lista de productos desde Supabase con filtros opcionales.
    🔗 FLUTTER: GET /productos?categoria=X&busqueda=Y
    """
    try:
...
def obtener_comentarios(producto_id: int):
    """
    🔗 FLUTTER: GET /productos/{producto_id}/comentarios
    """
    try:
...
def publicar_comentario(data: ComentarioCreate, user_id: Optional[str] = None):
    """
    🔗 FLUTTER: POST /productos/comentarios
    """
