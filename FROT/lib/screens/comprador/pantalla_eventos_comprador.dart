// lib/screens/comprador/pantalla_eventos_comprador.dart
//
// Pantalla de calendario de eventos para el COMPRADOR: permite descubrir
// ferias, talleres, exposiciones, bazares y festivales artesanales en todo
// el país, filtrar por categoría/provincia, elegir un día en el calendario
// y ver el detalle + contacto del organizador de cada evento.
...
class PantallaEventosComprador extends StatefulWidget {
  final String userId;

  const PantallaEventosComprador({super.key, this.userId = ''});

  @override
  State<PantallaEventosComprador> createState() =>
      _PantallaEventosCompradorState();
}

class _PantallaEventosCompradorState extends State<PantallaEventosComprador> {
  ...
  List<EventoArtesanal> get _eventosFiltrados {
    ...
    if (_diaSeleccionado != null) {
      final delDia = lista
          .where((ev) => ev.ocurreEnDia(_diaSeleccionado!))
          .toList();
      ...
      final cercanos = lista
          .where((ev) => !ev.soloFechaFin.isBefore(_diaSeleccionado!))
          .toList();
  ...
  Widget _buildCalendario() {
    return CalendarioMensual(
      ...
      alSeleccionarDia: (dia) {
        setState(() {
          if (dia.month != _mesMostrado.month ||
              dia.year != _mesMostrado.year) {
...
class _BotonVerMapa extends StatefulWidget {
  final VoidCallback onTap;
  const _BotonVerMapa({required this.onTap});

  @override
  State<_BotonVerMapa> createState() => _BotonVerMapaState();
}

class _BotonVerMapaState extends State<_BotonVerMapa> {
  bool _hover = false;
